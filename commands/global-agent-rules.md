...
- **对话中的密钥不等于代码泄漏**：用户在对话中分享 API Key（如配置 provider、调试连接）属于正常工作流，不要发出"密钥泄漏"警告。仅当密钥被写入源代码文件时才告警。前端已做脱敏展示，无需反复提醒。
...
Skills 存储在 `~/.codex/skills/`（个人）和可选的 `.codex/skills/`（项目共享）。

开始任务前：

- 扫描可用的 skills。
- 如果某个 skill 匹配当前任务，读取其 `SKILL.md` 并遵照执行。
- 声明正在使用哪些 skill。
...
| 场景 | Skill | 触发词 |
|------|-------|--------|
| 网络调研 / 文档查询 / 事实核查 | `researcher` | "search"、"research"、"搜索"、"调研"、"查文档"、"查一下"、"帮我搜"，Context7 + 网络搜索 |
